...
- **Param-typed-only and property dispatchers.** A closure registered on a dispatcher that arrives as a typed `function boot(Dispatcher $dispatcher)` parameter, or via a typed property `$this->dispatcher->listen(...)`, is not matched. The container-resolved receiver shapes (above) are matched; these two are the same [known gaps as for class listeners](listeners.md#known-limitations).
...
2. Is the call shape `listen(EventName, Closure)` against the `Event::` facade or a [container-resolved dispatcher](listeners.md#container-form-registrations)? Both are matched. A dispatcher arriving as a typed `boot(Dispatcher $dispatcher)` parameter or via `$this->dispatcher` is a known gap.
